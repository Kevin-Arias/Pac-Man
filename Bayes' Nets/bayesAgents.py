...
def adjacent_house_center(gameState, obsPos):
    distance = float("inf")
    adj_house = None
    bottomLeftPos, topLeftPos, bottomRightPos, topRightPos = gameState.getPossibleHouses()
    houses = []
    houses.append(bottomLeftPos)
    houses.append(topLeftPos)
    houses.append(bottomRightPos)
    houses.append(topRightPos)
    for house in houses:
        manhattan = util.manhattanDistance(obsPos, house)
        if distance > manhattan:
            adj_house = house
            distance = manhattan
    if adj_house == bottomLeftPos:
        return BOTTOM_LEFT_VAL
    if adj_house == topLeftPos:
        return TOP_LEFT_VAL
    if adj_house == bottomRightPos:
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in possible_houses:
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                probability = 0
                adjacent = adjacent_house_center(gameState, obsPos)
                ghost_house_val, food_house_val, wall_color = assignment[GHOST_HOUSE_VAR], assignment[FOOD_HOUSE_VAR], assignment[obsVar]
                if adjacent != ghost_house_val and adjacent != food_house_val:
                    if wall_color == NO_OBS_VAL:
                        probability = 1
                elif adjacent == food_house_val or ghost_house_val == food_house_val:
                    if wall_color == RED_OBS_VAL:
                        probability = PROB_FOOD_RED
                    elif wall_color == BLUE_OBS_VAL:
                        probability = 1 - PROB_FOOD_RED
                elif adjacent == ghost_house_val:
                    if wall_color ==  RED_OBS_VAL:
...
def getBestOption(factor):
    best_option = None
    best_probability = 0
    for assignment in factor.getAllPossibleAssignmentDicts():
        assignment_probability = factor.getProbability(assignment)
        if best_probability < assignment_probability:
            best_option = assignment
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    best_foodhouse_position = getBestOption(factor)
    return best_foodhouse_position
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        foodleft_ghostright = 0
        foodright_ghostleft = 0
        ...
        for assignment in assignments:
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                foodleft_ghostright = factor.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                foodright_ghostleft = factor.getProbability(assignment)

        left_won_reward = foodleft_ghostright * WON_GAME_REWARD
        left_ghost_reward = (1 - foodleft_ghostright) * GHOST_COLLISION_REWARD
        right_won_reward = foodright_ghostleft * WON_GAME_REWARD
        right_ghost_reward = (1 - foodright_ghostleft) * GHOST_COLLISION_REWARD
        leftExpectedValue = left_won_reward + left_ghost_reward
        rightExpectedValue = right_won_reward + right_ghost_reward
        #util.raiseNotDefined()
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        probs_and_outcomes = self.getExplorationProbsAndOutcomes(evidence)
        for prob, outcome in probs_and_outcomes:
            temp_evidence = {}
            for key, value in evidence.iteritems():
                temp_evidence[key] = value
            temp_evidence.update(outcome)
            updated_evidence = temp_evidence
            action1, action2 = self.computeEnterValues(updated_evidence, enterEliminationOrder)
            max_action = max(action1, action2)
            expectedValue += prob * max_action
        #util.raiseNotDefined()
