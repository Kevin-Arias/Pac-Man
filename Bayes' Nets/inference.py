...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for variable in eliminationOrder:
            all_factors, resulting_factor = joinFactorsByVariable(all_factors, variable)
            if len(resulting_factor.unconditionedVariables()) == 1:
                continue
            else:
                eliminated_factor = eliminate(resulting_factor, variable)
                all_factors.append(eliminated_factor)
        temp = joinFactors(all_factors)
        normalized_factors = normalize(temp)
